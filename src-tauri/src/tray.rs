...
fn current_provider_submenu_label(header_label: &str, provider_name: &str) -> String {
    format!("{header_label} · {provider_name}")
}
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        if providers.is_empty() {
            ...
            let submenu_label = match current_provider {
                Some(p) => current_provider_submenu_label(section.header_label, &p.name),
...
#[cfg(test)]
mod tests {
    use super::{current_provider_submenu_label, TRAY_ID};
    ...
    #[test]
    fn current_provider_submenu_label_omits_usage_suffix() {
        let label = current_provider_submenu_label("Claude", "Test Provider");
        assert_eq!(label, "Claude · Test Provider");
